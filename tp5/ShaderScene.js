...
export class ShaderScene extends CGFscene {
	...
	init(application) {
		...
		this.testShaders = [
			...
			new CGFshader(this.gl, "shaders/windowHeight.vert", "shaders/windowHeight.frag"),
			new CGFshader(this.gl, "shaders/sinAnim.vert", "shaders/sinAnim.frag"),
	...
	onScaleFactorChanged(v) {
		...
		this.testShaders[this.selectedExampleShader].setUniformsValues({ timeFactor: performance.now() / 1000 });
	}
	...
	display() {
		...
		if(this.selectedExampleShader == this.shadersList['Water']){
			...
			this.testShaders[this.selectedExampleShader].setUniformsValues({ uSampler2: 1 });			
		}
		else{
